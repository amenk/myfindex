...
uses ReadDiskUnit, NewCollectionUnit, NewListUnit,
  ExportOptUnit, ListCompareOptUnit, SetupUnit, ListviewConfig,
...
function AddFileIconToIL(il: TImageList; FileName: string; flags: uint): integer;
var
  icnTemp: TIcon;
  SFI: TSHFileInfo;
begin
  SHGetFileInfo(PChar(FileName), 0, SFI, SizeOf(TSHFileInfo), SHGFI_ICON or flags);
  icnTemp := TIcon.Create;
  icnTemp.Handle := SFI.hIcon;
  Result := il.AddIcon(icnTemp);
  icnTemp.Free;
end;

function AddResIconToIL(il: TImageList; id: integer): integer;
var
  icnTemp: TIcon;
begin
  icnTemp := TIcon.Create;
  icnTemp.Handle := LoadIcon(hInstance, PChar(id));
  Result := il.AddIcon(icnTemp);
  icnTemp.Free;
end;
...
begin
  ...
  try
    ...
    while res = 0 do
    begin
      if dontload.indexof(sr.Name) = -1 then
        try
  ...
  begin
    Item := TMenuItem.Create(menNew);
    Item.Caption := FPlugIns.AllProps[i];
    Item.OnClick := genNewFieldClick;
    menNew.Add(Item);
  end;
...
//  lvstyle := 1;
  ...
  idi_root := AddResIconToIL(ilMoreImages, 501);
  idi_disk := AddResIconToIL(ilMoreImages, 502);
  idi_nodisk := AddResIconToIL(ilMoreImages, 503);

  idi_closef := AddFileIconToIL(ilMoreImages, extractfilepath(application.exename), SHGFI_SMALLICON);
  idi_openf := AddFileIconToIL(ilMoreImages, extractfilepath(application.exename), SHGFI_SMALLICON or SHGFI_OPENICON);
...
begin
  ...
  begin
    ...
    begin
      flder := LookUp('FOLDERID', folderid, 'Folder');
      if Caption <> '' then flder := flder + Caption + '\';
      // flder := '\';
      Filter := 'Folder = ''' + escape(flder) + '''';
      Filtered := True;
      none_available := True;
      First;
      while not eof do
      begin
        if Pos(',' + tblFoldersDISKID.AsString + ',', avdiskids) <> 0 then
        begin
...
var
  ...
  begin
    MS := dm.tblFiles.CreateBlobStream(bf, bmRead);
  ...
  begin
    ...
    case data.Item.typ of
      lvt_verbund: begin
          ...
          begin
            IndexName := '';
            s := dm.tblFolders.LookUp('FOLDERID', data.item.id.folderid, 'Folder') + item.Caption + '\';
            setkey;
            tblFoldersDISKID.Value := -1;
            tblFoldersFolder.Value := s;
            if Gotokey then
              mNotes.Text := tblFoldersNote.AsString else
            begin
              Append;
              tblFoldersDISKID.Value := -1;
              tblFoldersFolder.Value := s;
...
procedure TMyFiles3Form.StepWait;
var
  s : string;
begin
  Inc(FWaitInt);
